// ============================================================
// apps-view.js — AI 下游·应用与服务 视图渲染 + 筛选逻辑
// ============================================================
// 与 upstream-view.js / downstream-view.js 完全平行
//   substage：商业化阶段（Pre-Rev/早起商业/规模盈利）
//   moat + pricingPower（护城河×定价权框架）
//
// 依赖：window.APPS_DATA (apps.js)
// 暴露：window.initAppsView()
...
(function () {
  ...
  const GROUP_ZH = { 'B2B应用': 'B2B 企业应用', 'B2C平台': 'B2C 消费平台' };
  ...
  function renderGroupBadge(group) {
    const clsMap = { 'B2B应用': 'materials', 'B2C平台': 'metals' };
  ...
  function updateKPIs(filteredCatIds) {
    ...
    const elCats = $('#apps-kpi-cats');
    const elCos = $('#apps-kpi-companies');
    const elL3 = $('#apps-kpi-l3');
    const elElasticity = $('#apps-kpi-elasticity');
    const elFiltered = $('#apps-kpi-filtered');
    ...
    if (elL3) elL3.innerHTML = `<span class="upstream-kpi-value">${stats.l3Categories}</span><span class="upstream-kpi-sub">L3 规模盈利 · L2 ${stats.l2Categories} · L1 ${stats.l1Categories}</span>`;
  ...
  function bindEvents() {
    ...
    $$('#apps-table-head .sortable').forEach(th => {
      th.addEventListener('click', () => {
        const key = th.dataset.sort;
        if (state.sortKey === key) { state.sortDir *= -1; }
        else { state.sortKey = key; state.sortDir = -1; }
        renderTable();
        $$('#apps-table-head .sortable').forEach(t => t.classList.remove('sorted-asc', 'sorted-desc'));
