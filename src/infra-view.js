// ============================================================
// infra-view.js — AI 中游·算力基础设施 视图渲染 + 筛选逻辑
// ============================================================
// 与 upstream-view.js / downstream-view.js 完全平行
//   substage：运营成熟度（建设期/爬坡期/成熟运营）
//   moat + pricingPower（与下游相同的护城河×定价权框架）
//
// 依赖：window.INFRA_DATA (infra.js)
// 暴露：window.initInfraView()
...
(function () {
  ...
  function stageLabel(stage) {
    if (stage === 'L3') return '成熟运营';
    if (stage === 'L2') return '爬坡期';
    if (stage === 'L1') return '建设期';
    return '';
  }
  ...
  function renderGroupBadge(group) {
    const clsMap = { '算力运营': 'metals', '数字基建': 'materials' };
  ...
  function filteredCategories() {
    ...
    if (state.moatFilter) {
      ids = ids.filter(id => categories[id].moat === state.moatFilter);
    }
    if (state.pricingFilter) {
      ids = ids.filter(id => categories[id].pricingPower === state.pricingFilter);
    }
    if (state.marketFilter) {
      ids = ids.filter(id => {
        return categories[id].companies.some(cid => {
          const co = D().companies[cid];
          return co && co.market === state.marketFilter;
        });
      });
    }
    if (state.searchQuery) {
      ...
      ids = ids.filter(id => {
        ...
        return cat.companies.some(cid => {
          const co = D().companies[cid];
          return co && (co.name.toLowerCase().includes(q) || co.ticker.toLowerCase().includes(q));
        });
      });
    }
  ...
  function renderTable() {
    ...
    sorted.forEach(catId => {
      ...
      // 显示第一家公司的市场
      const marketBadge = cos.length > 0 ? renderMarketBadge(cos[0].market) : '';
  ...
  function updateKPIs(filteredCatIds) {
    ...
    const elCats = $('#infra-kpi-cats');
    const elCos = $('#infra-kpi-companies');
    const elL3 = $('#infra-kpi-l3');
    const elElasticity = $('#infra-kpi-elasticity');
    const elFiltered = $('#infra-kpi-filtered');
    ...
    if (elL3) elL3.innerHTML = `<span class="upstream-kpi-value">${stats.l3Categories}</span><span class="upstream-kpi-sub">L3 成熟 · L2 ${stats.l2Categories} · L1 ${stats.l1Categories}</span>`;
    ...
    if (elFiltered) {
      ...
      elFiltered.textContent = `筛选: ${filteredCatIds.length} 赛道 · ${state.searchQuery ? '搜索 "' + state.searchQuery + '"' : ''}`;
    }

    // 更新备注行
    const footnote = $('#infra-last-updated');
    if (footnote && D().meta) {
      footnote.textContent = `数据版本 ${D().meta.version} · 方法论文档见 infra-methodology.md · 弹性系数 = DOL × PricingPower · ${stats.totalCategories} 赛道 · ${stats.totalCompanies} 家公司 · Vibe-Trading web_search 辅助筛选`;
    }
  }
  ...
  function bindEvents() {
    ...
    const searchInput = $('#infra-search');
    if (searchInput) {
    ...
    $$('#infra-table-head .sortable').forEach(th => {
      th.addEventListener('click', () => {
        ...
        renderTable();
        $$('#infra-table-head .sortable').forEach(t => t.classList.remove('sorted-asc', 'sorted-desc'));
